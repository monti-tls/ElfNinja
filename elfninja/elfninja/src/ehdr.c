...
static enjp_ehdr_command* _commands = 0;
static enjp_ehdr_command* _last_command = 0;

enjp_ehdr_command* enjp_ehdr_commands()
{
    return _commands;
}

enjp_ehdr_command* enjp_ehdr_resolve_command(const char* name, enj_error** err)
{
    if (!name)
    {
    ...
    for (enjp_ehdr_command* cmd = _commands; cmd; cmd = cmd->next)
    {
...
int enjp_ehdr_register_command(enjp_ehdr_command* cmd, enj_error** err)
{
    ...
    if (enjp_ehdr_resolve_command(cmd->name, 0))
    {
...
int enjp_ehdr_help(enji_cmdline* cmd)
{
    ...
    {
        enjp_ehdr_command* cmd = enjp_ehdr_resolve_command(arg->name->string, 0);
        if (!cmd)
        {
...
int enjp_ehdr_run(enji_cmdline* cmd)
{
    ...
    if (!file)
        enjp_fatal(0, "No file specified. Try 'elfninja ehdr help'");
    ...
    if (!file->next)
    {
        enjp_error(0, "No command specified. Try 'elfninja ehdr help'");
...
{
    "ehdr",
    "Manipulate ELF file header and header tables",
    &enjp_ehdr_help,
    &enjp_ehdr_run
};

static __attribute__((constructor(104))) void _register()
{
    enj_error* err = 0;

    if (enjp_tool_register(&_this_tool, &err) < 0)
        enjp_fatal(&err, "Unable to register tool 'ehdr'");
}
