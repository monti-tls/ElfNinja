...
int enjp_data_insert_help()
{
    return 0;
}

int enjp_data_insert_run(enjp_data_tool* d, enji_cmdline_argument* arg, enj_error** err)
{
    ...
    if (enjp_data_read_input(d, arg, "insert", err) < 0)
    {
        enjp_error(err, "Bad input parameters");
        return -1;
    }

    enjp_message("Inserting %ld bytes at file offset 0x%08lX", d->effective_length, d->file_offset);
    ...
    for (size_t i = 0; i < d->count; ++i)
    {
        ...
        if (enj_blob_insert(d->elf->blob, off, d->bytes, length, err) < 0)
        {
            enjp_error(err, "Unable to insert data into blob");
            return -1;
        }
    }
...
{
    "insert",
    "Insert data to the ELF file",
    &enjp_data_insert_help,
    &enjp_data_insert_run
};
...
static __attribute__((constructor(202))) void _register()
{
    ...
    if (enjp_data_register_command(&_this_cmd, &err) < 0)
        enjp_fatal(&err, "Unable to register data command 'insert'");
}
