...
static enjp_shdr_command* _commands = 0;
static enjp_shdr_command* _last_command = 0;

enjp_shdr_command* enjp_shdr_commands()
{
    return _commands;
}

enjp_shdr_command* enjp_shdr_resolve_command(const char* name, enj_error** err)
{
    if (!name)
    {
    ...
    for (enjp_shdr_command* cmd = _commands; cmd; cmd = cmd->next)
    {
...
int enjp_shdr_register_command(enjp_shdr_command* cmd, enj_error** err)
{
    ...
    if (enjp_shdr_resolve_command(cmd->name, 0))
    {
...
int enjp_shdr_help(enji_cmdline* cmd)
{
    ...
    {
        enjp_shdr_command* cmd = enjp_shdr_resolve_command(arg->name->string, 0);
        if (!cmd)
        {
...
int enjp_shdr_run(enji_cmdline* cmd)
{
    ...
    if (!file)
        enjp_fatal(0, "No file specified. Try 'elfninja shdr help'");
    ...
    if (!file->next)
    {
        enjp_error(0, "No command specified. Try 'elfninja shdr help'");
    ...
    size_t count = write(fd, s.elf->blob->buffer, s.elf->blob->buffer_size);
    if (count != s.elf->blob->buffer_size)
    {
        if (count < 0)
...
{
    "shdr",
    "Manipulate ELF section headers",
    &enjp_shdr_help,
    &enjp_shdr_run
};

static __attribute__((constructor(105))) void _register()
{
    enj_error* err = 0;

    if (enjp_tool_register(&_this_tool, &err) < 0)
        enjp_fatal(&err, "Unable to register tool 'shdr'");
}
