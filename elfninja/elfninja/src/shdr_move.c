...
int enjp_shdr_move_help()
{
    return 0;
}

int enjp_shdr_move_run(enjp_shdr_tool* s, enji_cmdline_argument* arg, enj_error** err)
{
    ...
    enji_cmdline_option* opt = enji_cmdline_find_option(s->cmd, "to", ENJI_CMDLINE_TOOL, arg, 0);
    if (!opt || !opt->value)
    {
    ...
    if (enj_elf_shdr_move(source, dest->index, err) < 0)
    {
        enjp_error(err, "Unable to move section header");
        goto fail;
    }
...
static __attribute__((constructor(204))) void _register()
{
    ...
    if (enjp_shdr_register_command(&_this_cmd, &err) < 0)
        enjp_fatal(&err, "Unable to register section command 'move'");
}
